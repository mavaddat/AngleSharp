...
- Do not head `CHANGELOG.md` with an `## Unreleased` marker or similar: `ReleaseNotesParser`
  requires its first line to parse as SemVer, and a non-version heading aborts the Fallout
  build. The top heading is the version currently *in flight*, not the last one shipped: it is
  opened by the first change after a release, carrying that release's planned date and the
  matching `<Version>` bump. So add new bullets under it even once that date has passed, and
  leave opening the next heading to the maintainer.
